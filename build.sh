...
if [ "${1:-release}" == "dev" ]; then
    ...
    node build/scripts/release-app.mjs | node_modules/.bin/uglifyjs --config-file build/scripts/minify.config.json -o build/release/main.js
    last_line=$(node_modules/.bin/roadroller build/release/main.js -O2 -Zab0 -Zlr1064 -Zmc4 -Zmd17 -Zpr16 -S0,1,2,3,7,13,14,21,42,57,289,452 -o build/release/main.js 2>&1 | tail -n 1)
    log "$last_line"
    rm -rf dist
    mkdir -p dist/src
    node_modules/.bin/html-inline -i build/release/index.html -o dist/src/index.html > /dev/null
    if [ $PLATFORM == "windows" ]; then
        ./tools/7z/7za a -tzip dist/game.zip ./dist/src/index.html > /dev/null 2>&1
    	./tools/ect -9 -zip dist/game.zip > /dev/null 2>&1
        ./tools/cloc-1.86.exe --quiet --hide-rate src/
    else
        ./tools/7z/7zz a -tzip dist/game.zip ./dist/src/index.html > /dev/null 2>&1
    	./tools/ect -9 -zip dist/game.zip > /dev/null 2>&1
